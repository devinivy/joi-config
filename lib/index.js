    ...
    return (joi) => {
        ...
        return {
            ...
            prepare(val, { schema, state, prefs, original }) {
                ...
                // We want to stash all the schemas that need resolving and
                // then remove them from the value that is going to be returned
                // so that only resolved values are handed back to the user.

                let stash;

                if (keys.length) {
                    stash = Hoek.clone(value, { shallow: true });
                    value = Hoek.clone(value, { shallow: true });
                }

                for (const key of keys) {
                    // Per the previous comment, this is where we massage
                    // value so it's suitable to be returned to the user.
                    delete value[key];
                }

                for (const key of keys) {
                    const subschema = stash[key];
                ...
                value: {
                    method(value, opts = {}) {
                        ...
                        for (const [key] of Traverse.findPaths(compiled, isChildSchema)) {
                            ...
                            topo.add(key, { group: key, after: schema.$_rootReferences() });
                        }
                    ...
                    method(opts = {}) {
                        ...
                        if ('switch' in opts) {
                            ...
                            if (Array.isArray(opts.switch)) {
                                ...
                                if (opts.switch.length === 0) {
                                    // Ensure to strip rather than pass thru
                                    opts.switch = [{ is: joi.any(), then: joi.strip() }];
                                }
                            }
