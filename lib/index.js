        ...
        return {
            ...
            prepare(_, { schema, state, prefs, original }) {
                ...
                value = internals.mapSchemaValues(joi, value, (s, path, parents) => {
                    ...
                    if (typeof validate.value === 'undefined') {
                        return internals.strip;
                    }
...
internals.mapSchemaValues = (joi, val, fn, path = [], parents = []) => {
    ...
    if (Array.isArray(val)) {
        ...
        val.forEach((value, i) => {
            ...
            const result = internals.mapSchemaValues(joi, value, fn, [...path, i], [arr, ...parents]);

            if (result !== internals.strip) {
                arr.push(result);
            }
        });
        return arr;
    ...
    Object.entries(val).forEach(([key, value]) => {
        ...
        const result = internals.mapSchemaValues(joi, value, fn, [...path, key], [obj, ...parents]);
