...
module.exports = (() => {
    ...
    return (joi) => {
        ...
        internals.decorate(joi, 'pref', (path, opts) => joi.ref(path, defaultRefOptions(opts)));
        internals.decorate(joi, 'pin', (path, opts) => joi.in(path, defaultRefOptions(opts)));
        internals.decorate(joi, 'pexpression', (path, opts) => joi.expression(path, defaultRefOptions(opts)));
        internals.decorate(joi, 'px', (path, opts) => joi.x(path, defaultRefOptions(opts)));

        return {
            type: /^/,
            prepare(val, { schema, state, prefs, original }) {

                if (!state.ancestors[internals.original]) {
            ...
            rules: {
                ...
                param: {
                    method(path, opts) {
                        ...
                        return this.value(joi.pref(path, opts));
                    }
                },
                whenParam: {
                    method(path, opts) {

                        return this.when(joi.pref(path), opts);
                    }
                },
                intoWhen: {
                ...
                into: {
                    method(map = new Map()) {
                        ...
                        const buildSchema = (x) => {
                            ...
                            if (typeof x === 'undefined' || joi.isSchema(x)) {
                                return x;
                            }
